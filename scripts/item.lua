...
--  Brutal Oblige Notes
------------------------------------------------------------------------
--The brutal oblige module was causing problems with this script.
--I couldn't for the life of me work out why so I took the scorched earth option and included a custom version of the script.
...
function Item_pickups_for_class(CL)
  ...
  local function decide_pickup(R, stat, qty)
	...
	if table.empty(item_tab) then --force the itme_tab to be populated rather than error if it is not
		item_tab={bullets=20}
	end
	
    gui.printf("item_tab:\n" .. table.tostring(item_tab).."\n")
