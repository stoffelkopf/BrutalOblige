--curent highest used id number 324 (Zyberdemon)
...
function BRUTALDOOM.gameinfo()
  ...
  table.insert(data,'LOAD="brutalv20b.pk3","bfriend1.pk3"')
...
Intermission BrutalDoomCast
  ...
  each L in GAME.levels do
    ...
    if L.description and prefix then --keep the prefix stuff from the bex incase something else relies on it
      ...
      local sky1 = 'sky1 = "RSKY1' --default sky. Final " deliberately ommitted, see the roll for extra sky bit.
	...
	local n = rand.irange(1,3)
      ...
      --roll for extra sky
      if BRUTALDOOM.PARAMETERS.skygenerator == true and rand.irange(1,2) == 2 and theme_name != "satanshankerchief" then
          sky1 = sky1 .. 'B'
      end
...
--logos
function STARTERPACK.logos()
    gui.wad_name_gfx('TEST', 'Hello world!', 1);
    ...
    PREFABS.NukemBanner1 =
        {
        file   = "brutaloblige/nukembanner.wad"
        where  = "edge"
        long   = 64
        deep   = 16
    ...
    gui.printf('\nPrefabs:\n');
    gui.printf(table.tostring(PREFABS));
    gui.printf('\n');
